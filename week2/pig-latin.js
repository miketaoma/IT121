...
for (let i = 0; i < inputTextArray.length; i++) { //for loop iterates the if statements below through each element of array.
    ...
    } else if (vowels.includes(firstChar)) { //tests if first character of user input is a vowel, appends "way" if true.
        translatedText = inputTextArray[i] + "way";
        document.write(translatedText);
    } else if (vowels.includes(secondChar)) { //tests if second character of user input is a vowel (if test above fails we know first char is a consonant), appends that consonant + "ay" if true. Including "y" bec of edgecase words like "by" or "mystery"
